...
class button implements ActionListener{
   ...
   //out how to get the data from the search results and print it out in text format from the search engine)
   ...
   class test extends button implements ActionListener{
      ...
      public static ArrayList boolCheck = new ArrayList();
      static class nestedClass extends textBox{
         ...
         static class nestedClassTwo extends secondMainClass{
            public static void nestedMethodTwo() {
               ...
               if(boolCheck.size() == 0){
                  try{
                     ...
                     File weatherText = new File("weatherinfo.txt");
                     //if(weatherText.delete()){System.out.println("file deleted");}
                     //else{System.out.println("no such file");}
                     helloMethod();
                     methodScanner(filehello , howAreYou, weatherText, n1);
               ...
               try{
                  String enteredBox = textBox.getTextBox(showBox);
                  store1.clear();
                  store1.add(enteredBox);
                  helloMethod();
                  Runtime runTime = Runtime.getRuntime();
                  Process process = runTime.exec(store1.get(0));
                  Label launchingRespTwo = new Label("Bot: Launching " + store1.get(0));
                  printerMethodCreator.printerMethodCreatorTwo.pMethod(500,60,launchingRespTwo,myF,5,250,20,0,195,0,x);
                  for(int i = 0; i<=37; i++){store.add(1);}
                  boolCheck.clear();
               }
               catch(IOException e){
                  try{
                     Label notWorking = new Label("Bot: filepath/program not found");
                     printerMethodCreator.printerMethodCreatorTwo.pMethod(500,60,notWorking,myF,5,250,20,0,195,0,x);
                     boolCheck.clear();
                  }
                  catch(FileNotFoundException es){System.out.println("...........");}
               }
            }
            }

            //moved the changeStr code into that method and now it does not print the correct statement(think this is fixed)
            public static void methodScanner(File one, File two,File three, String string){
                 Random randIntegerResponse = new Random();
                 int randomIntegerResoponse = randIntegerResponse.nextInt(3)+0;
                 ArrayList<String> responses = new ArrayList<String>();
                 responses.add("Bot:I Don't Understand");
                 responses.add("Bot:Not coded for that scenerio");
                 responses.add("Bot:Let's talk about something else");
                 File responsesFile = new File("responses.txt");
                 File filehellobot = new File("hellobot.txt");
                 //File weatherGetter = new File(
                 try{
                   ...
                   if(changeStr(three).contains(string)){
                     textGrabber("https://forecast.weather.gov/MapClick.php?lat=48.50379000000004&lon=-122.23361999999997");
                   }
                   if(string.equals("launch")){
                     launchProgram();
                   }
                   else{
                     Label t = new Label(responses.get(randomIntegerResoponse));
            ...
            public static void launchProgram() {
               try{
               Label launchingResp = new Label("Bot: Type in the filepath/program name");
               printerMethodCreator.printerMethodCreatorTwo.pMethod(500,60,launchingResp,myF,5,250,20,0,195,0,x);
               boolCheck.add(1);
               }
               catch(FileNotFoundException e){System.out.println("AHH");}
               
            }
            
            public static void textGrabber(String url){
               try{
               ArrayList weatherPos = new ArrayList();
               File fileW = new File("weatherResponse.txt");
               Document doc = Jsoup.connect(url).get();
               String ownText = doc.body().text();
               weatherPos.add(ownText);
               if(fileW.delete()){System.out.println("FILE DELETED");}
               else{System.out.println("FILE NOT FOUND");}
               FileWriter writer = new FileWriter("weatherResponse.txt",true);
               writer.write(ownText);
               writer.close();
               Scanner readingFile = new Scanner(new File("weatherResponse.txt"));
               ArrayList listWeather = new ArrayList();
               while(readingFile.hasNext()){
                  listWeather.add(readingFile.next());
               }
               readingFile.close();
               int pos = listWeather.indexOf("°F");
               String weather = "Sedro's current temp is: " + listWeather.get(pos-1) + listWeather.get(pos);
               System.out.println(weather);
               File lastWeather = new File("lastWeather.txt");
               FileWriter writerTwo = new FileWriter("lastWeather.txt");
               writerTwo.write(weather);
               printerMethodClass.printerMethodClassTwo.showingMethodTwo(weather);
               }
               catch(IOException e){System.out.println("ahhhhhhh");}
            }
...
class printerMethodClass extends test{
   ...
   public static class printerMethodClassTwo extends frameGraphics{
       ...
       public static void showingMethodTwo(String string) throws FileNotFoundException{
        ...
        // while (s.hasNext()){
         ...
         //   }
         //s.close();
         Label machinePrintTwo = new Label("Bot: " + string);
         printerMethodCreator.printerMethodCreatorTwo.pMethod(500,60,machinePrintTwo,myF,5,250,20,0,195,0,x);
       }
   }
}
